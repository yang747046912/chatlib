package com.kaws.lib.xrecyclerview;
...
public class XRecyclerView extends RecyclerView {
    ...
    private KAWSrefreshHeader mRefreshHeader;
    ...
    private void init(Context context) {
        if (pullRefreshEnabled) {
            KAWSrefreshHeader refreshHeader = new KAWSrefreshHeader(context);
    ...
    public void addHeaderView(View view) {
        if (pullRefreshEnabled && !(mHeaderViews.get(0) instanceof KAWSrefreshHeader)) {
            KAWSrefreshHeader refreshHeader = new KAWSrefreshHeader(getContext());
            mHeaderViews.put(0, refreshHeader);
            mRefreshHeader = refreshHeader;
        }
    ...
    private void addFootView(final View view) {
        mFootViews.clear();
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0
                    && lastVisibleItemPosition >= layoutManager.getItemCount() - 1 && layoutManager.getItemCount() > layoutManager.getChildCount() && !isnomore && mRefreshHeader.getState() < KAWSrefreshHeader.STATE_REFRESHING) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isOnTop() && pullRefreshEnabled) {
                    ...
                    if (mRefreshHeader.getVisiableHeight() > 0 && mRefreshHeader.getState() < KAWSrefreshHeader.STATE_REFRESHING) {
                        return false;
                    }
                }
                break;
