...
public class AddBankCardActivity extends LoadBaseActivity {

    private ImageView imgAddBankcardBankicon;
    private TextView tvSelectBankcard;
    private TextView tvAddBankcardTip;
    private Button btnEnsureAddBankcard;
    private EditText edtAddBankcardNumber;
    private EditText edtAddBankcardName;
    private EditText edtAddBankcard_branch;
    private RelativeLayout rlAddBankcardBank;
    // 银行卡号
    ...
    private void setUpView() {
        btnEnsureAddBankcard.setOnClickListener(listener);
        rlAddBankcardBank.setOnClickListener(listener);
        edtAddBankcardNumber.setOnClickListener(listener);
        btnEnsureAddBankcard.setClickable(false);
        edtAddBankcardNumber.addTextChangedListener(numberWatcher);
        edtAddBankcard_branch.addTextChangedListener(textWatcher);
        edtAddBankcardName.addTextChangedListener(textWatcher);
        setServiceTextColor();
        initAccountInfo();
    }
    ...
    private void initAccountInfo() {
        if (getIntent() != null) {
            ...
            String accountNumber = cards.getString(Constants.TAKE_MONEY_ACCOUNT_NUMBER);
            String bankName = cards.getString(Constants.TAKE_MONEY_BANK_NAME);
            String accountName = cards.getString(Constants.TAKE_MONEY_ACCOUNT_NAME);
            String subBranch = cards.getString(Constants.TAKE_MONEY_SUBBRANCH);

            edtAddBankcardNumber.setText(accountNumber);
            tvSelectBankcard.setText(bankName);
            for (int i = 0; i < Constants.banks.length; i++) {
                if (Constants.banks[i].equals(bankName)) {
                    imgAddBankcardBankicon.setImageResource(Constants.resIds[i]);
                    imgAddBankcardBankicon.setVisibility(View.VISIBLE);
                }
            }
            /** 若没有账户名则显示医生名*/
            if (TextUtils.isEmpty(accountName)) {
                edtAddBankcardName.setText(SQuser.getInstance().getUserInfo().info.name);
            } else {
                edtAddBankcardName.setText(accountName);
            }
            edtAddBankcard_branch.setText(subBranch);
        }
    }

    private void initView() {
        edtAddBankcardNumber = (EditText) findViewById(R.id.edt_add_bankcard_number);
        edtAddBankcardName = (EditText) findViewById(R.id.edt_add_bankcard_name);
        tvSelectBankcard = (TextView) findViewById(R.id.tv_select_bankcard);
        tvAddBankcardTip = (TextView) findViewById(R.id.tv_add_bankcard_tip);
        rlAddBankcardBank = (RelativeLayout) findViewById(R.id.rl_add_bankcard_bank);
        edtAddBankcard_branch = (EditText) findViewById(R.id.edt_add_bankcard_branch);
        btnEnsureAddBankcard = (Button) findViewById(R.id.btn_ensure_add_bankcard);
        imgAddBankcardBankicon = (ImageView) findViewById(R.id.img_add_bankcard_bankicon);
        imgAddBankcardBankicon.setVisibility(View.INVISIBLE);
    }
    ...
    private PerfectClickListener listener = new PerfectClickListener() {
        ...
        protected void onNoDoubleClick(View v) {
            ...
            Intent intent;
            switch (id) {
                case R.id.btn_ensure_add_bankcard://确定添加
                    if (!checkBankCard(bankCardNumber)) {
                        TipDialog.hintDiaglog(AddBankCardActivity.this, "请输入正确的卡号");
                        return;
                    }
                    addBankCard();
                    break;
                case R.id.rl_add_bankcard_bank://选择银行
                    intent = new Intent(AddBankCardActivity.this, SelectBankCardActivity.class);
                    startActivityForResult(intent, Constants.CODE_BANKCARD_ICON);
                    break;
            }
        }
    };
    ...
    private void addBankCard() {
        //银行、姓名、支行
        String bankName = tvSelectBankcard.getText().toString();
        String accountName = edtAddBankcardName.getText().toString();
        String subBranch = edtAddBankcard_branch.getText().toString();

        startProgressDialog();
        HttpService service = HttpUtils.getInstance().getHttpService(HttpService.class);
        SQuser sQuser = SQuser.getInstance();
        service.addBankcard(HttpHead.getHeader("POST"), sQuser.getUserInfo().token, sQuser.getUserInfo().doctorid, bankCardNumber, bankName, accountName, subBranch, new CustomCallBack<RestError>() {

            @Override
            public void onSuccess(RestError restError) {
                stopProgressDialog();
                TipDialog tipDialog = new TipDialog(AddBankCardActivity.this);
                tipDialog.setMessage("添加银行卡成功");
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == Constants.CODE_BANKCARD_ICON) {//选择发卡银行返回
            int icon = data.getIntExtra(Constants.SELECT_BANK_ICON, 0);
            String name = data.getStringExtra(Constants.SELECT_BANK_NAME);
            imgAddBankcardBankicon.setVisibility(View.VISIBLE);
            imgAddBankcardBankicon.setImageResource(icon);
            tvSelectBankcard.setText(name);
        }
    }
    ...
    private void isClicked() {
        ...
        if ((bankCardNumber.length() > 15 && bankCardNumber.length() < 20) && (!"".equals(edtAddBankcard_branch.getText().toString())) && (!"".equals(tvSelectBankcard.getText().toString())) && (!"".equals(edtAddBankcardName.getText().toString()))) {
            btnEnsureAddBankcard.setSelected(true);
            btnEnsureAddBankcard.setClickable(true);
        } else {
            btnEnsureAddBankcard.setSelected(false);
            btnEnsureAddBankcard.setClickable(false);
        }
    }

    /**
     * 根据银行卡号设置银行 及 设置"确定提交" 按钮的点击事件
     */
    private void setButtonClickable() {
        bankCardNumber = edtAddBankcardNumber.getText().toString().replaceAll(" ", "");

        if (bankCardNumber.length() >= 6) {
            char[] cardNumber = bankCardNumber.substring(0, 6).toCharArray();
            String name = new BankInfo().getNameOfBank(cardNumber, 0);//获取银行卡的信息
            rlAddBankcardBank.setClickable(true);
            for (int i = 0; i < Constants.banks.length; i++) {

                if (name.contains(Constants.banks[i])) {
                    tvSelectBankcard.setText(Constants.banks[i]);
                    imgAddBankcardBankicon.setImageResource(Constants.resIds[i]);
                    imgAddBankcardBankicon.setVisibility(View.VISIBLE);
                }
                if (name.contains("中信")) {
                    tvSelectBankcard.setText(Constants.banks[7]);
                    imgAddBankcardBankicon.setImageResource(Constants.resIds[7]);
                    imgAddBankcardBankicon.setVisibility(View.VISIBLE);
                }
                if (name.contains("广东发展")) {
                    tvSelectBankcard.setText(Constants.banks[13]);
                    imgAddBankcardBankicon.setImageResource(Constants.resIds[13]);
                    imgAddBankcardBankicon.setVisibility(View.VISIBLE);
                }
                if (name.contains("浦东发展")) {
                    tvSelectBankcard.setText(Constants.banks[8]);
                    imgAddBankcardBankicon.setImageResource(Constants.resIds[8]);
                    imgAddBankcardBankicon.setVisibility(View.VISIBLE);
                }
            }
        }
     ...
     * 客服字体颜色
     */
    private void setServiceTextColor() {
        String ServiceNumber = tvAddBankcardTip.getText().toString();
        ...
        builder1.setSpan(span, 26, 38, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        tvAddBankcardTip.setText(builder1);
    }
    ...
    private TextWatcher numberWatcher = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            // TODO Auto-generated method stub
            if (isChanged) {
                location = edtAddBankcardNumber.getSelectionEnd();
                int index = 0;
                ...
                edtAddBankcardNumber.setText(str);
                Editable etable = edtAddBankcardNumber.getText();
    ...
    public class BankInfo {
        ...
        //二分查找方法
        public int binarySearch(long[] srcArray, long des) {
            int low = 0;
